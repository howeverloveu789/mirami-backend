// src/core/report/q19_report.js

// 備註：Q19Scores / Q19Answers 型別註解只留在說明裡，不用 TS

// 這裡 key 要跟 scoring 裡保持一致
const MisjudgmentGroupKeys = [
  'patience_threshold',
  'rhythm_instability',
  'dosage_illusion',
  'signal_misread',
...
/**
 * 主入口：由 runQ19.js 呼叫
 * @param {object} scores  // 內含 groups / essenceGrabScore 等
 * @param {object} answers // { [id: number]: string | 'A' | 'B' | 'C' | 'D' }
 */
function buildQ19Report(scores, answers) {
  const rawGroups =
    scores.groups ||
    scores.misjudgmentGroups ||
    [];

  /** @type {{ key:string, score:number, label:'low'|'mild'|'medium'|'high' }[]} */
 ...
 * 1) 身分感模組（節奏版）
 */
function buildIdentitySnippets(scores, groups) {
  const snippets = [];

  const essenceGrabScore = scores.essenceGrabScore ?? 0;
  const chaosToSystemScore = scores.chaosToSystemScore ?? 0;
  const pressureFocusScore = scores.pressureFocusScore ?? 0;
  const wasteSensitivityScore = scores.wasteSensitivityScore ?? 0;
 ...
 * @param {Array<{tag:string,title:string}>} snippets
 */
function buildThinkingRoutes(snippets) {
  if (!snippets.length) {
 ...
 */
function buildIdentityIntro(snippets) {
  if (!snippets.length) {
    return [
...
function mapGroupKeyToLabel(key) {
  switch (key) {
...
function buildTrapSentence(group) {
 ...
 */
function buildThinkingProfile(scores, groups) {
  const lines = [];
 ...
 */
function buildLifeScenes(answers) {
  const lines = [];

  const regret = answers[62];
  const impulsive = answers[63];
 ...
 */
function buildEarlyWarnings(groups) {
  const lines = [];
